...
fprintf('objg: SD, Problem size: '); fprintf('%d\n',length(x.p));
tic, [inform,xnew] = SteepDescent(@objg,x,nparams); toc,
if inform.status == 0
...
fprintf('objg: SteepDescent, Problem size: '); fprintf('%d\n',length(x.p));
tic, [inform,xnew] = SteepDescent(@objg,x,nparams); toc,
if inform.status == 0
  fprintf('CONVERGENCE FAILURE: %d steps were taken without\n', inform.iter);
  fprintf('gradient size decreasing below %8.4g.\n\n', nparams.toler);
...
tic, [inform,xnew] = SteepDescent(@objampl,x,nparams); toc,
if inform.status == 0
  fprintf('CONVERGENCE FAILURE: %d steps were taken without\n', inform.iter);
  fprintf('gradient size decreasing below %8.4g.\n\n', nparams.toler);
else
